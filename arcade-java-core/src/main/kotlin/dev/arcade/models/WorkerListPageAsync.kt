...
private constructor(
    private val workersService: WorkerServiceAsync,
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is WorkerListPageAsync && workersService == other.workersService && params == other.params && response == other.response /* spotless:on */
    }

    override fun hashCode(): Int = /* spotless:off */ Objects.hash(workersService, params, response) /* spotless:on */

    override fun toString() =
        "WorkerListPageAsync{workersService=$workersService, params=$params, response=$response}"
    ...
    fun getNextPage(): CompletableFuture<Optional<WorkerListPageAsync>> {
        return getNextPageParams()
            .map { workersService.list(it).thenApply { Optional.of(it) } }
    ...
    companion object {
        ...
        @JvmStatic
        fun of(workersService: WorkerServiceAsync, params: WorkerListParams, response: Response) =
            WorkerListPageAsync(workersService, params, response)
    }
    ...
    class AutoPager(private val firstPage: WorkerListPageAsync) {

        fun forEach(
            action: Predicate<WorkerResponse>,
            executor: Executor,
        ): CompletableFuture<Void> {
            fun CompletableFuture<Optional<WorkerListPageAsync>>.forEach(
                action: (WorkerResponse) -> Boolean,
                executor: Executor,
            ): CompletableFuture<Void> =
                thenComposeAsync(
                    { page ->
        ...
        fun toList(executor: Executor): CompletableFuture<List<WorkerResponse>> {
            val values = mutableListOf<WorkerResponse>()
