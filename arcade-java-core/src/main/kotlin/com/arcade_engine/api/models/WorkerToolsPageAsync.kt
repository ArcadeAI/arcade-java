...
private constructor(
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is WorkerToolsPageAsync && workersService == other.workersService && params == other.params && response == other.response /* spotless:on */
    }

    override fun hashCode(): Int = /* spotless:off */ Objects.hash(workersService, params, response) /* spotless:on */

    override fun toString() =
        "WorkerToolsPageAsync{workersService=$workersService, params=$params, response=$response}"
    ...
    fun getNextPageParams(): Optional<WorkerToolsParams> {
        if (!hasNextPage()) {
            return Optional.empty()
        }

        return Optional.of(
            WorkerToolsParams.builder()
                .from(params)
    ...
    fun getNextPage(): CompletableFuture<Optional<WorkerToolsPageAsync>> {
        return getNextPageParams()
            .map { workersService.tools(it).thenApply { Optional.of(it) } }
    ...
    companion object {
        ...
        @JvmStatic
        fun of(workersService: WorkerServiceAsync, params: WorkerToolsParams, response: Response) =
            WorkerToolsPageAsync(workersService, params, response)
    }
    ...
    class AutoPager(private val firstPage: WorkerToolsPageAsync) {

        fun forEach(
            action: Predicate<ToolDefinition>,
            executor: Executor,
        ): CompletableFuture<Void> {
            fun CompletableFuture<Optional<WorkerToolsPageAsync>>.forEach(
