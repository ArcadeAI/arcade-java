...
sealed class JsonField<out T : Any> {
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw ArcadeInvalidDataException("`$name` is not set")
            is JsonNull -> throw ArcadeInvalidDataException("`$name` is null")
            else -> throw ArcadeInvalidDataException("`$name` is invalid, received $this")
        }
